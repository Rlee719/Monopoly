...
preset prison [QASJ] [0~2]			#设置某玩家在监狱待n天
...
preset rounds [1~20]				#设置当前游戏的回合轮数，一轮包含各个玩家的一回合
preset is_bankrupt [QASJ] [0~1]		#设置某玩家是否出局，1表示设置该玩家为破产状态，0则反之
```
...
rounds:8					# 表示当前的游戏回合轮数，最大游戏回合数设置为20（暂时）
...
在执行`reset`命令,之后，保证你的游戏状态和以下dump一致。
...
接着执行`preset players QA`之后，保证你的游戏状态和以下dump一致。初始fund和points分别为10000，200。
...
- V1.4
  - 添加`house_level_0`到dump文件格式（被遗忘的项）

- V1.5
  - 添加游戏回合轮数`rounds`到dump文件格式，并添加相应的`preset`命令
  - 添加每个玩家的破产状态`is_bankrupt`到dump文件格式，并添加相应的`preset`命令
  - 修改拼写错误"prision"->"prison"
  - 修改 3.2 复位标准。
